...
queue_url = 'https://sqs.us-east-1.amazonaws.com/571815438302/fufillmentqueue'	
...
def lambda_handler(event, context):
    ...
    if pp==1:
        ...
        l=requests.get('https://maps.googleapis.com/maps/api/geocode/json?address='+city+'&key=AIzaSyBHQd71PM_KHxBvyuJkCpBFSJc8xAxG6BI')
        l1=json.loads(l.content)
        lat=l1['results'][0]['geometry']['location']['lat']
        lng=l1['results'][0]['geometry']['location']['lng']
        
        suggest=requests.get('https://maps.googleapis.com/maps/api/place/nearbysearch/json?location='+str(lat)+','+str(lng)+'&radius=1500&type=restaurant&keyword='+cuisine+'&key=AIzaSyBHQd71PM_KHxBvyuJkCpBFSJc8xAxG6BI')
    ...
    table = dynamodb.Table('fulfillment')
